...
void do_stabilize_reorder_pixelxy_simd(const tensor_t<double> & images, tensor_t<double> & output)
{
	OPEN_TRACE("trace.out");
	for (int this_frame = 1; this_frame < images.size.b; this_frame++) {
		int previous_frame = this_frame - 1;
		...
		{
			{
				...
				theDataCollector->disable_prefetcher();
				set_cpu_clock_frequency(1900);
				timer.attr("function", "do_stabilize_reorder_pixelxy_simd").go();
				do_stabilize_reorder_pixelxy_simd(batch_data, output);
			}
			CHECK_AND_RESET();
		}
